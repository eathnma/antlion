...
void drawMiniGame2() { 
  ...
  if (second() - goodTimer > good_interval) {
    goodfoods.add(new GoodFood(random(300,1100), random(100, 700), 35, 35));
    goodTimer = second();
  }
  
  if (second() - badTimer > bad_interval) {
    badfoods.add(new BadFood(random(300,1100), random(100, 700), 35, 35));
    badTimer = second();
  }
...
void mouseClicked() { 
  ...
  for (int i = 0; i < goodfoods.size(); i++) {
    GoodFood g = goodfoods.get(i);
    if (dist(mouseX, mouseY, g.x, g.y) < 18) {
      goodfoods.remove(i);
      acidX += 50;
    }
  }
  for (int i = 0; i < badfoods.size(); i++) {
    BadFood b = badfoods.get(i);
    if (dist(mouseX, mouseY, b.x, b.y) < 18) {
      badfoods.remove(i);
      acidX -= 50;
    }
  }
